...
double CTest::TestInit(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_init<int>(&first->atomic, 0);
	if(second != NULL) std::atomic_init<int>(&second->atomic, 0);
...
double CTest::TestStore(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_store(&first->atomic, 1);
	if(second != NULL) std::atomic_store(&second->atomic, 1);
...
double CTest::TestLoad(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL)  std::atomic_load(&first->atomic);	
	if(second != NULL) std::atomic_load(&second->atomic);
...
double CTest::TestFetchAdd(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_fetch_add(&first->atomic, 5);	
	if(second != NULL) std::atomic_fetch_add(&second->atomic, 5);	
...
double CTest::TestFetchSub(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_fetch_sub(&first->atomic, 2);
	if(second != NULL) std::atomic_fetch_sub(&second->atomic, 2);
...
double CTest::TestFetchAnd(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_fetch_and(&first->atomic, 9);	
	if(second != NULL) std::atomic_fetch_and(&second->atomic, 9);	
...
double CTest::TestFetchOr(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_fetch_or(&first->atomic, 9);	
	if(second != NULL) std::atomic_fetch_or(&second->atomic, 9);	
...
double CTest::TestFetchXor(AtomicType* first, AtomicType* second) {
	...
	if(first != NULL) std::atomic_fetch_xor(&first->atomic, 9);	
	if(second != NULL) std::atomic_fetch_xor(&second->atomic, 9);	
